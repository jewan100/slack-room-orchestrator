...
### 2.5 명령 처리 신뢰성 규칙
- Slash command 응답은 "실제 영속 상태"와 모순되면 안 된다.
- 핵심 단계(세션 선점/상태 전이/핵심 저장)와 후속 단계(안내 메시지/요약 공지)를 분리한다.
- 후속 단계 실패는 command 전체 실패로 승격하지 않고 경고 로그로 분리 기록한다.
- 상태 전이는 원자적으로 선점한다. (`WHERE state = ...` 조건 + 영향 row 검증)
...
- 내부 예외 원문(error.message/stack)은 사용자 응답에 노출 금지
- 내부 예외 원문은 로그 컨텍스트 필드로만 기록
...
- `elapsedMs`
- `errorCode` (오류 시)
- `internalErrorMessage` (내부 오류 원문, 사용자 응답 비노출)

금지:
- 토큰/API 키/민감정보 원문 로그 출력

운영 안정성:
- 종료 시그널 처리(`SIGINT`/`SIGTERM`)는 single-flight 가드 적용
- 종료 비동기 실패는 `catch` 후 오류 로그 + 비정상 종료 코드로 처리
