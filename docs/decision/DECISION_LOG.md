...
- 결정:
  ...
  - Slack slash 응답 UX는 "ephemeral 1개"로 고정(ack 1회 + 성공 delete_original best-effort + 실패 replace_original)
  - 에러 교체 책임은 inbound adapter 1곳으로 고정하고 도메인 핸들러는 throw만 수행
...
- 근거:
  ...
  - Slack 3초 ack 제한에서 "Slackbot 실패 메시지"와 중복 ephemeral이 반복 발생해 UX가 깨짐
  - 운영자 관점의 상태/응답 모순과 재시도 혼선을 사전 차단할 필요
- 후속 액션:
  - 신규 커맨드(`status`, `decide`, 택배/소포 확장 도메인) 구현 시 동일 규칙 적용
  - Slash UX 단일화 패턴을 표준 템플릿으로 재사용
...
### D-006A
- 날짜: 2026-02-13
- 주제: 회의 종료 UX 최소화(스레드 중심)
- 영향 범위: 회의 종료 UX, 채널 노이즈, 기록 위치
- 결정:
  - 회의 종료(`/room stop`/TTL 만료)는 planning 스레드에 **한 줄 종료 안내만** 남긴다.
  - 종료 시 채널 본문에는 별도 공지/요약을 게시하지 않는다(도배 방지).
  - 긴 리포트(요약/정리/액션아이템)는 v1 범위에서 제외하고, 필요하면 2차에서 별도 커맨드(`/room end` 등)로 분리한다.
- 근거:
  - 채널 본문 노이즈를 최소화해야 사용자가 체감하는 UX가 안정됨
  - 종료 기록은 원래 대화가 진행된 planning 스레드에 남겨야 맥락이 유지됨
- 후속 액션:
